...
const AppMenuitem = (props: AppMenuItemProps) => {
    ...
    const itemClick = (event: React.MouseEvent<HTMLAnchorElement, MouseEvent>) => {
        ...
        if (item!.items) {
            setSubmenuActive(!submenuActive); // Toggle the submenu state
        }
    };

    const subMenu = item!.items && item!.visible !== false && (
        <CSSTransition timeout={{ enter: 1000, exit: 450 }} classNames="layout-submenu" in={submenuActive} key={item!.label}>
            <ul>
